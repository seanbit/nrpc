...
build-cli:
	@$(MKFOLDER)
	@go build -o build/$(BIN) github.com/seanbit/nrpc/pitaya-cli
	@echo "build pitaya-cli at ./build/$(BIN)"

build-k6-extension:
	@$(MKFOLDER)
	@xk6 build --with github.com/topfreegames/xk6-pitaya=./xk6-pitaya/ --with github.com/seanbit/nrpc=./ --with google.golang.org/grpc=google.golang.org/grpc@v1.54.1 --output ./build/$(XK6_BIN)
...
mocks: serializer-mock metrics-mock cluster-mock nrpc-mock

nrpc-mock:
	@mockgen github.com/seanbit/nrpc NRpc | sed 's/mock_nrpc/mocks/' > mocks/app.go

metrics-mock:
	@mockgen github.com/seanbit/nrpc/metrics Reporter | sed 's/mock_metrics/mocks/' > metrics/mocks/reporter.go
	@mockgen github.com/seanbit/nrpc/metrics Client | sed 's/mock_metrics/mocks/' > metrics/mocks/statsd_reporter.go

serializer-mock:
	@mockgen github.com/seanbit/nrpc/serialize Serializer | sed 's/mock_serialize/mocks/' > serialize/mocks/serializer.go

cluster-mock:
	@mockgen github.com/seanbit/nrpc/cluster RPCClient,RPCServer,SDListener,InfoRetriever | sed 's/mock_cluster/mocks/' > cluster/mocks/cluster.go
